(function () {
   ...
   * The Markdown is generated by another content script that intercepts
   * Notion network requests and cached in the background service worker.
   * This helper first asks the background for cached markdown. If none is
   * available yet, it waits a little while for the interceptor to finish.
   *
   * @returns {Promise<string|null>}
   */
  async function getNotionPageContent() {
    const isNotionPage = /(?:www\.)?notion\.so$/.test(location.hostname);
    if (!isNotionPage) return null;

    // Extract the raw 32-character page ID from the URL. Example:
    // https://www.notion.so/My-Note-2317aa7407c180c5b0e8d3cd45615e52
    const pageId = location.pathname.split('/').pop()?.split('-').pop() || '';
    if (!pageId) return null;

    /**
     * Ask the background script for cached markdown.
     * @returns {Promise<string|null>}
     */
    const requestCachedMarkdown = async () => {
      try {
        const res = await chrome.runtime.sendMessage({
          type: 'get-notion-page-markdown',
          pageId,
        });
        if (res && typeof res.markdown === 'string' && res.markdown.length) {
          return res.markdown;
        }
      } catch (_) {
        /* ignore */
    ...
    // 1️⃣ Try immediate cache lookup
    let markdown = await requestCachedMarkdown();
    if (markdown) return markdown;

    // 2️⃣ Wait for incoming markdown broadcast from the interceptor or cache
    //     population for a short period (up to ~8s) before giving up.
    markdown = await new Promise((resolve) => {
      ...
      // Fallback timeout – stop waiting after TIMEOUT_MS
      ...
      // Periodically poll the cache in the background in case we missed the
      // broadcast message (e.g., it arrived before we attached the listener).
      const pollId = setInterval(async () => {
        const md = await requestCachedMarkdown();
        if (md) {
      ...
      const listener = (msg) => {
        if (
          msg?.type === 'notion-page-chunks-markdown' &&
          msg.data?.pageId === pageId &&
          typeof msg.data.markdown === 'string' &&
          msg.data.markdown.length
        ) {
          clearInterval(pollId);
          clearTimeout(timeoutId);
